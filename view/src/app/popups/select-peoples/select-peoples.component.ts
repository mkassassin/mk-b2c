...
@Component({
  selector: 'app-select-peoples',
  templateUrl: './select-peoples.component.html',
  styleUrls: ['./select-peoples.component.css']
})
export class SelectPeoplesComponent implements OnInit {
  ...
  constructor(private router: Router,
    ...
    private dialogRef: MatDialogRef<SelectPeoplesComponent>,
    @Inject(MAT_DIALOG_DATA) private data: any ) {

        this.UserInfo = JSON.parse(localStorage.getItem('currentUser'));
        this.ActiveCategory = this.data.ActiveCategory;
        this.Header = this.data.Header;
        this.Service.UnFollowingUsers(this.UserInfo.data._id, this.data.ActiveCategory)
        .subscribe( userdatas =>  {
  ...
  close() {
    this.dialogRef.close({status: 'Next'});
  }
